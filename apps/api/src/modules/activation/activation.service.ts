...
export class ActivationService {
  ...
  private async buildBaselineMetrics(data: SiteData): Promise<BaselineMetricsDto> {
    ...
    return {
      ...
      indexablePages: null,
      noindexPages: null,
      criticalIssues: data.issues.filter((issue) => issue.severity === 'CRITICAL').length,
      highIssues: data.issues.filter((issue) => issue.severity === 'HIGH').length,
      mediumIssues: data.issues.filter((issue) => issue.severity === 'MEDIUM').length,
      lowIssues: data.issues.filter((issue) => issue.severity === 'LOW').length,
      rankingQueries: null,
